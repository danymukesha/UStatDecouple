...
This vignette demonstrates how to use UStatDecouple with real genomic
data from Bioconductor packages, including DNA sequences and gene
expression datasets.

## 1. Introduction

This vignette shows practical applications of UStatDecouple using
real-world biological datasets from Bioconductor. We’ll analyze:
...
We’ll use the `BSgenome.Hsapiens.UCSC.hg19` package to load human
...
Now we’ll analyze whether these promoter regions have similar nucleotide
composition patterns, which could indicate shared regulatory mechanisms.
...
We’ll use the `ALL` dataset from the `ALL` package, which contains gene
expression data from acute lymphoblastic leukemia patients.
...
We’ll analyze whether genes show co-expression patterns, indicating
potential functional relationships or co-regulation.
...
We’ll create a custom kernel to analyze the similarity of genomic
features based on their overlap patterns.
...
We’ll create a kernel that measures the similarity between genes based
...
This vignette demonstrated how to apply UStatDecouple to real-world
genomic data:

1.  **DNA sequences**: Analyzed promoter diversity to detect shared
    regulatory features
2.  **Gene expression**: Identified co-expression patterns in leukemia
    patients
3.  **Genomic features**: Examined spatial organization of genes across
    chromosomes

Key takeaways:

- Always preprocess data appropriately (normalize, handle missing
  values)
...
- Integrate results with other Bioconductor tools for comprehensive
  biological interpretation

For more theoretical background and simpler examples, see the “Getting
Started with UStatDecouple” vignette.
